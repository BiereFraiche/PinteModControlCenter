# PinteMod Control Center — candidat recette v2.4.5-rc26

La version stable publique reste `2.4.4`. Le candidat `2.4.5-rc26` prépare la recette terrain finale : résultat RCON affiché immédiatement, fermeture locale bornée, assistant RCON optionnel uniquement pour un serveur vierge, remplacement confirmé d’un RCON existant sans l’afficher, ouverture automatique du tableau de bord après lancement, Agent LAN activable explicitement, interface compatible session distante et dossier autonome. Un silence RCON reste distinct de l’état local PinteMod.
...
```text
app\artifacts\release-v2.4.5-rc26-win-x64\single-exe\PinteMod.ControlCenter.exe
app\artifacts\release-v2.4.5-rc26-win-x64\folder\
app\artifacts\release-v2.4.5-rc26-win-x64\PinteMod.ControlCenter-single-exe-win-x64.zip
app\artifacts\release-v2.4.5-rc26-win-x64\PinteMod.ControlCenter-folder-win-x64.zip
app\artifacts\release-v2.4.5-rc26-win-x64\SHA256SUMS.txt
app\artifacts\release-v2.4.5-rc26-win-x64\SELF-TEST.txt
```
...
Le build lance automatiquement l’auto-diagnostic hors ligne. Il peut aussi être relancé depuis l’EXE unique ou depuis le dossier :

```powershell
.\PinteMod.ControlCenter.exe --self-test --self-test-report="C:\Temp\PinteMod-self-test.txt"
```

Il vérifie la version, les assemblages, les six vues WPF et les payloads embarqués sans charger de profil serveur, de secret DPAPI, de réseau ou de transport RCON. La page **Paramètres** propose le même contrôle et permet de copier son rapport anonymisé.
