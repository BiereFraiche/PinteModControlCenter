...
internal sealed class ReadOnlyRankJsonFileReader(RankRecordsPathPolicy pathPolicy)
{
    private const int MaximumAttempts = 3;

    public Task<LocalJsonFileResult<T>> ReadAsync<T>(
        RankRecordsDirectory directory,
        string path,
        ...
        where T : class =>
        Task.Run(
            () => ReadOnWorkerAsync(directory, path, maximumFileSizeBytes, parser, cancellationToken),
            cancellationToken);
    ...
    private async Task<LocalJsonFileResult<T>> ReadOnWorkerAsync<T>(
        ...
        where T : class
    {
        var authorizedPath = pathPolicy.ValidateFilePath(directory, path);
