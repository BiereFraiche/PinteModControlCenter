...
Le bridge PinteMod Control Center Runtime v0.1.2 fournit désormais un snapshot persistant borné à quatre joueurs observables et huit armes par joueur. Le Control Center conserve le BOIII_XUID complet uniquement dans ses modèles internes de ciblage et n’affiche que sa forme abrégée. Le pseudo reste informatif. Les données absentes du contrat — notamment l’état Mute autoritaire — restent inconnues et ne sont pas inventées. Les contrats encore manquants concernent les capacités par carte, ChangeMap/RestartMap, événements/boss génériques et le feedback unifié des mutations.
